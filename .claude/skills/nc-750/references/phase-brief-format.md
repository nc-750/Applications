...
The canonical artifact `nc-750-plan` emits. It is consumed by `nc-750-challenge` (plan mode) and,
...
- **Tests are descriptions, and they must survive the critic.** Each test description should be able
  ...
  `challenge-report-format.md` for how `nc-750-challenge` interrogates these.)
...
`nc-750-plan` emits the brief → `nc-750-challenge` (plan mode) returns a challenge report → loop
