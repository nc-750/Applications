...
Mirror keeps your data local. None of the items below are uploaded to a Mirror-owned server. Mirror has no server.

**Settings** — Your selected provider, model, endpoint, and (in the PWA) your API key.

**Mirror data** — Your finished mirror output.
...
**Clear mirror data** — Wipes the mirror data and interview stores. Your provider settings and API key are kept.

**Clear AI provider** — Clears the API key from the app. Your mirror data and interview data are kept.

**Factory reset** — Full wipe: all mirror data, interview, and API key data is deleted. The app then reloads to its first-run state.
