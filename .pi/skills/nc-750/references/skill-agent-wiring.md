...
You (the orchestrator) own `references/` because it is the composition root. Role skills **cite**
these files (e.g. "emit the phase plan per `../nc-750/references/phase-plan-format.md`"); they do
not restate the schemas.
...
- **Agent** (`.pi/agents/nc-750-<verb>.md`) = a thin wrapper. Frontmatter pins the **model** and a
  **tool allowlist**; the body is ~3 lines: "You are the NC-750 `<verb>` role. Load and follow the
