...
import { Switch } from "enclosure-vue";
...
async function handleExport() {
    ...
    downloadFile(json, "persona.json", "application/json");
    logger.info("export", "Persona exported");
}
...
async function handleImport() {
    ...
    input.onchange = async () => {
        ...
        try {
            ...
            logger.error("import", "Persona import failed", {
...
async function handleClearPersonaData() {
    if (
        !confirm(
            "This deletes your persona and interview history. Your AI provider settings stay. Continue?",
        )
    )
        return;
    await wipePersonaData();
    flashCleared("Persona data cleared.");
}

async function handleClearAiProvider() {
    if (
        !confirm(
            "This deletes your API key and provider settings. Your persona stays. Continue?",
        )
    )
        return;
        ...
        >
            ...
            >
                <div :style="{ padding: 'var(--nc-space-5)' }">
                    ...
                    >
                        This deletes your persona, interview history, AI
