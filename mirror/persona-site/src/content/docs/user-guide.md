---
title: "User Guide"
description: "Step-by-step walkthrough for getting the most out of Persona"
order: 1
---

A step-by-step walkthrough for getting the most out of Persona.

## What Persona does

Persona interviews you about your career, then produces two documents:
...
Everything stays on your device. There is no account and no Persona server. You bring your own AI key.

---

## 1. Set up your AI provider

Persona needs an AI provider to run the interview. You supply the key — the app talks to the provider directly from your machine.
...
See the [Provider Setup Guide](/docs/provider-setup) for help getting API keys.
...
If your input is very large (roughly over 600,000 characters), Persona will run a background analysis pass to condense it before the interview. You'll see a notice when this happens.
...
When the interview is complete, the AI will thank you and Persona will build your profile. This takes a few seconds.
...
## 4. Import an existing persona.json

If you already have a persona.json file (from another device or an earlier export):

1. From the welcome screen, click **Import persona.json**.
2. Select the file.
3. If the file is valid, your persona loads immediately — no re-interview needed.

You can also import from **Settings → Data → Import persona.json**.
...
- **persona.json:** go to **Settings → Data → Export persona.json**. This exports the raw structured data, which can be re-imported later.
...
Persona stores everything locally. The Settings panel has three data controls:

- **Clear persona data** — deletes your persona and interview history. Keeps your provider settings.
- **Clear AI provider** — removes your API key and provider settings. Keeps your persona.
